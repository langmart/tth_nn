...
class OneHotMLP:
    ...
    def _plot_roc_curve(self, arr_true, arr_pre, arr_weights, epoch, indicator):
        ...
        plt.title(r'Receiver operating characteristic (ROC)', fontsize=20)
        plt.xlabel(r'Signal efficiency', fontsize=16)
        plt.ylabel(r'Background rejection', fontsize=16)
        plt.legend(loc='best')
        plt.xlim(0, 1.05)
        plt.ylim(0, 1.05)
        ax = plt.gca()
    ...
    def _plot_auc_dev(self, train_auc, val_auc, epochs):
        ...
        plt.ylabel(r'AUC score', fontsize=16)
        ax = plt.gca()
        plt.setp(ax.get_xticklabels(), fontsize=15)
        plt.setp(ax.get_yticklabels(), fontsize=15)
