...
class OneHotMLP:
    ...
    def train(self, train_data, val_data, epochs = 10, batch_size = 100,
        ...
        with train_graph.as_default():
            ...
            x_m = tf.Variable(np.mean(train_data.x, axis=0).astype(np.float32),
                    trainable=False)
            x_d = tf.Variable(np.std(train_data.x, axis=0).astype(np.float32), trainable=False)
            x_scaled = tf.div(tf.sub(x, x_m), x_d)
